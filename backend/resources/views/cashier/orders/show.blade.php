...
<div class="space-y-6 max-w-3xl mx-auto" id="order-detail" data-order-id="{{ $order->id }}">
    ...
    <div class="card py-4" id="status-tracker">
        ...
        @endphp
        {{--
            Setiap circle & label pakai id="tracker-circle-N" dan id="tracker-label-N"
            agar JS bisa update tanpa bergantung pada class (class-based querySelector
            gagal bila class berubah atau ada class lain di elemen yang sama).
            Garis connector pakai id="tracker-line-N" (N = 0..3, antara step N dan N+1).
        --}}
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-green-600 mt-0.5">
                            Konfirmasi ketika tamu sudah selesai makan dan meja kosong
                        </p>
                    </div>
                    <form method="POST"
            ...
            <div class="bg-green-50 border border-green-300 rounded-xl px-4 py-4">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-green-600 mt-0.5">
                            Konfirmasi ketika pelanggan sudah mengambil pesanan
                        </p>
                    </div>
                    <form method="POST"
                          action="{{ route('cashier.orders.complete', $order) }}"
        ...
        <div class="card">
            ...
            <dl class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <dd class="text-indigo-600 font-medium">
                        {{ $order->sent_to_kitchen_at->format('H:i') }}
                    </dd>
                </div>
                @endif
            </dl>
        ...
        <div class="card">
            ...
            <dl class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <dd class="text-green-600 font-semibold">
                        Rp {{ number_format($order->totalPaid(),0,',','.') }}
                    </dd>
                </div>
                <div class="flex justify-between">
            ...
            <div id="payment-actions">
                ...
                @elseif($order->isFullyPaid() && $order->isCompleted())
                    {{--
                        FIX #4 & #5: target="_blank" agar halaman print terbuka di tab baru,
                        halaman show tetap aktif dan bisa memantau progres.
                    --}}
                ...
                @elseif($order->isFullyPaid() && !$order->isCompleted())
                    {{-- Sudah lunas tapi belum selesai: tetap tampilkan tombol cetak --}}
                    <a href="{{ route('cashier.receipts.print', $order->payments->last()) }}"
                       target="_blank"
                       rel="noopener noreferrer"
                       class="w-full justify-center mt-4 inline-flex items-center gap-2 px-4 py-2.5
    ...
    <div class="flex gap-3">
        ...
        {{-- FIX #2: buka modal cancel, bukan form inline dengan hardcoded reason --}}
        <button type="button"
...
{{-- ── MODAL CANCEL ── --}}
{{--
    FIX #2: modal dengan textarea alasan + tombol alasan cepat.
    CancelOrderRequest rules: cancel_reason required|string|max:500
    Form submit POST ke cashier.orders.cancel
--}}
<div id="cancel-modal"
    ...
    <div style="background:#fff; border-radius:16px; padding:24px; width:100%;
        ...
        <form method="POST" action="{{ route('cashier.orders.cancel', $order) }}" id="cancel-form">
            @csrf
            <div class="mb-3">
...
(function () {
    ...
    // Semua badge class yang mungkin — untuk di-strip sebelum menambah yang baru
    ...
    // Hitung step aktif berdasarkan status + is_paid
    ...
    /*
     * FIX #3: update tracker menggunakan id="tracker-circle-N", bukan class selector.
     * Setiap elemen di-set ulang className-nya secara penuh sehingga
     * tidak bergantung pada state class sebelumnya.
     */
    function updateTracker(currentStep) {
        for (let i = 0; i <= 4; i++) {
            ...
            // Garis hanya ada di antara step 0-3 (id tracker-line-0 s/d tracker-line-3)
            const line   = document.getElementById('tracker-line-' + i);
            ...
            // Garis N aktif jika step N sudah selesai DAN step N+1 juga sudah
            // (yaitu: i < currentStep, bukan i <= currentStep)
            if (line) {
                line.className =
    ...
    async function pollOrder() {
        try {
            ...
            // Reload halaman jika status berubah agar action buttons ikut terupdate
            // (complete button, payment button, dll tergantung PHP render)
            if (statusChanged) {
                window.location.reload();
            }

        } catch (e) {
            // Diam saja jika gagal poll
        }
    }
