...
<div style="display:flex; gap:0; height:calc(100vh - 64px); overflow:hidden;"
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <div style="background:#fff; border:1.5px solid #e5e7eb; border-radius:12px; overflow:hidden; cursor:pointer; transition:border-color 0.15s, box-shadow 0.15s, transform 0.15s; user-select:none;"
                            ...
                            <div style="height:110px; background:#f8fafc; overflow:hidden; position:relative;">
                                @if($product->image)
                                    <img src="{{ Storage::url($product->image) }}" alt="{{ $product->name }}" style="width:100%; height:100%; object-fit:cover; display:block;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; color:#d1d5db;"><svg xmlns="http://www.w3.org/2000/svg" style="width:36px;height:36px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg></div>
                                @else
                                    <div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:#d1d5db;"><svg xmlns="http://www.w3.org/2000/svg" style="width:36px;height:36px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg></div>
                                @endif
                                @if($hasVariants)<div style="position:absolute; top:6px; right:6px; background:#6366f1; color:white; font-size:9px; font-weight:700; padding:2px 7px; border-radius:8px; pointer-events:none;">{{ $variants->count() }} variasi</div>@endif
                                @if($hasDiscount)<div style="position:absolute; top:6px; left:6px; background:#ef4444; color:white; font-size:9px; font-weight:700; padding:2px 7px; border-radius:8px; pointer-events:none;">{{ $activeDisc->type === 'percentage' ? number_format($activeDisc->value,0).'%' : 'DISKON' }} OFF</div>@endif
                            </div>
                            <div style="padding:10px 12px 12px;">
                                ...
                                @if($product->description)<p style="font-size:11px; color:#9ca3af; margin:0 0 6px; line-height:1.3; display:-webkit-box; -webkit-line-clamp:1; -webkit-box-orient:vertical; overflow:hidden;">{{ $product->description }}</p>@else<div style="margin-bottom:6px;"></div>@endif
                                <div style="display:flex; align-items:center; justify-content:space-between; gap:4px;">
                                    <div>
                                        <span style="font-size:13px; font-weight:700; color:#4f46e5;">@if($hasVariants) Mulai @endif Rp {{ number_format($displayPrice, 0, ',', '.') }}</span>
                                        @if($hasDiscount)<span style="font-size:10px; color:#9ca3af; text-decoration:line-through; display:block; margin-top:1px;">Rp {{ number_format($product->price, 0, ',', '.') }}</span>@endif
                                    </div>
                                    @if($product->track_stock)<span style="font-size:10px; color:#9ca3af; background:#f3f4f6; padding:2px 6px; border-radius:10px; flex-shrink:0;">Stok {{ $product->stock }}</span>@endif
                                </div>
                            </div>
                        </div>
                        @endforeach
    ...
    <div style="width:320px; flex-shrink:0; display:flex; flex-direction:column; border-left:1px solid #e5e7eb; background:#fff; overflow:hidden;">
        ...
        <div style="flex:1; overflow-y:auto; padding:12px;">
            <div x-show="items.length === 0" class="flex flex-col items-center justify-center h-full text-gray-400 py-12">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 mb-2 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
            ...
            <div class="space-y-2">
                <template x-for="(item, index) in items" :key="item.key">
                    <div class="bg-gray-50 rounded-xl p-3">
                        ...
                        <template x-if="item.variant_id"><input type="hidden" :name="'items['+index+'][variant_id]'" :value="item.variant_id" form="order-form"></template>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-1.5">
                                <button type="button" @click="item.quantity > 1 ? item.quantity-- : removeItem(index)" class="w-7 h-7 rounded-full bg-white border border-gray-200 text-gray-600 text-sm font-bold hover:bg-gray-100 flex items-center justify-center">−</button>
                                <span class="text-sm font-bold w-6 text-center" x-text="item.quantity"></span>
                                <button type="button" @click="item.quantity++" class="w-7 h-7 rounded-full bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 flex items-center justify-center">+</button>
                            </div>
        ...
        <div style="padding:14px; border-top:1px solid #f1f5f9; flex-shrink:0; background:#fff;">
            <div x-show="items.length > 0">
                ...
                <button type="submit" form="order-form" class="btn-primary w-full justify-center py-3 text-base inline-flex items-center gap-2">
    ...
    {{-- Modal Varian — hanya varian, tidak ada "Standar" --}}
    <div :style="variantModal.open
        ...
        <div style="background:#fff; border-radius:16px; width:100%; max-width:420px; margin:16px; max-height:calc(100vh - 32px); box-shadow:0 24px 60px rgba(0,0,0,0.25); display:flex; flex-direction:column; overflow:hidden;">
            <div style="padding:16px 20px; border-bottom:1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between; flex-shrink:0;">
                ...
                <button type="button" @click="variantModal.open = false" style="color:#9ca3af; background:none; border:none; cursor:pointer; padding:4px; border-radius:6px; display:flex; align-items:center; justify-content:center;" onmouseover="this.style.color='#374151'; this.style.background='#f3f4f6';" onmouseout="this.style.color='#9ca3af'; this.style.background='none';">
                    ...
                    <div style="border:1.5px solid #e5e7eb; border-radius:10px; padding:12px 14px; margin-bottom:8px; cursor:pointer; background:#fff; transition:border-color 0.12s, background 0.12s;"
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <div style="text-align:right; flex-shrink:0; margin-left:12px;">
                                ...
                                <p x-show="Number(v.price_adjustment) !== 0" style="font-size:10px; color:#9ca3af; margin:2px 0 0;" x-text="(Number(v.price_adjustment)>0?'+':'-')+' Rp '+formatRp(Math.abs(Number(v.price_adjustment)))"></p>
                            </div>
                        </div>
                    </div>
                </template>
...
@push('scripts')
<script>
function orderForm(initial = [], taxRate) {
    return {
        items: initial,
        taxRate: taxRate,
